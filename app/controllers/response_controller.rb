class ResponseController < ApplicationController
  ...
  def redirect_when_disallowed(response)
    ...
    if response.map.read_attribute(:type) == 'FeedbackResponseMap' && response.map.assignment.team_count > 1 #Check if team count is more than 1 instead of checking if it is a team assignment
