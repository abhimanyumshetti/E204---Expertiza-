class Assignment < ActiveRecord::Base
  ...
  def contributors
    # Contributors are just teams, so removed check to see if it is a team assignment
    @contributors ||= teams #
  end

  def review_mappings
    # Reviews must be mapped just for teams, so removed check to see if it is a team assignment
    @review_mappings ||= team_review_mappings #
  end
  ...
  def review_mappings
    # Removed the if condition(and corressponding else) which differentiate assignments as team and individual assignments
  ...
  def get_contributor(contrib_id)
    #Always need to use the TEAM for contributors, so removed check to see if it is a team assignment 
...
def add_participant(user_name)
 ...
 def assign_reviewers(mapping_strategy)
   # Always assign reviewers for a team
